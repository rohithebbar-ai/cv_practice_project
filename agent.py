...
This drawing may contain FOUR distinct kinds of content. Extract ALL of them —
do not prioritize dimensions over the others. Each kind of content produces
its own row(s) in the output, distinguished by "Category".
...
=== 3. SPECIFICATION TABLES (Category="specification") ===
Extract every row of specification tables — "CONVEYOR CHARACTERISTICS",
"MATERIAL", "BELTING", or any PULLEY/IDLER/DRIVE/MOTOR/GEAR BOX sub-table.
This includes conveyor no., capacity, belt width/speed, horizontal length,
inclination, lift, feed/discharge points, angle of wrap, material handled,
bulk density, lump size, moisture content, tensions, belt designation,
splicing type, belt length, and every pulley/idler row with its dimensions.
For each row:
- Category="specification"
- Component_Name=the section/ITEM this row belongs to (e.g. "CONVEYOR", "MATERIAL", "BELTING", "PULLEY", "IDLER")
- Dim_Type="NA"
- Dim_Description=the exact PARTICULARS/DESCRIPTION label text from that row (e.g. "CAPACITY RATED/DESIGN (T.P.H)")
- Specified=the exact SPECIFICATION/value text from that row (e.g. "750/825")
- Quantity=the row's quantity/count if the table shows one (e.g. "28 NOS.", "1 NO."). Leave "" if the row has no quantity column (e.g. Belt Speed, Material Handled).
Do not skip rows. Do not leave Dim_Description or Specified blank if the table has content there.

=== 4. TITLE BLOCK (Category="title_block") ===
...
window.onload = function() {
    ...
    document.getElementById('addRowBtn').onclick = function() {
        ...
        var newRow = {
            ...
            Category: "dimension", Component_Name: "",
